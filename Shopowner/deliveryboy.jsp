...
<%
    if (request.getParameter("submit") != null) {
        ...
        if (password.equals(cpassword)) {
            String sel = "select * from tbl_login where login_email='" + email + "' and login_password='" + password + "'";
            ResultSet rs = con.select(sel);
            if (rs.next()) {
%>
<script>alert('This Delivery boy already Exist');</script>
<%
} else {
    String ins = "insert into tbl_login (login_email,login_password,login_role,login_status) values('" + email + "','" + password + "','deliveryboy','active')";
    boolean r = con.insert(ins);
    if (r) {
        String seld = "select * from tbl_login order by login_id desc limit 1";
        ResultSet rsd = con.select(seld);
        if (rsd.next()) {
            String deliveryboyid = rsd.getString("login_id");
            String insr = "insert into tbl_deliveryboy(d_name,d_address,d_phone,login_id,d_status,so_id) values('" + name + "','" + address + "','" + phone + "','" + deliveryboyid + "','active','"+session.getAttribute("ownerid")+"')";
            ...
            <div class="col-md-12">
                <div class="card">
                    <form class="form-horizontal" method="post">
                        <div class="card-body">
                            <h4 class="card-title">Add Delivery Boy here</h4>
